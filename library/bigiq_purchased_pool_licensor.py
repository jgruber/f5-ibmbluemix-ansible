    ...
    def exec_module(self):
        ...
        if self.module.check_mode:
            try:
                ...
                result['pool_uuid'] = PurchasedPoolLicensor._get_pool_id(  # pylint: disable=protected-access
                    biq.get_bigiq_session(),
                    self.module.params['bigiq_license_pool_name']
                )
                result['license_uuid'] = PurchasedPoolLicensor._get_member_id(  # pylint: disable=protected-access
                    biq.get_bigiq_session(),
                    result['pool_uuid'],
        ...
        try:
            ...
            member = F5BigIQLicensePoolMember(
                ...
                bigip_password=self.module.params[
                    'bigip_password'])
            licensor = PurchasedPoolLicensor(biq, member)
            if state == "present":
